...
def read_load_data(opt):
    ...
    train_set_3d, test_set_3d, data_mean_3d, data_std_3d, dim_to_ignore_3d, dim_to_use_3d, train_root_positions, test_root_positions,  bone_lengths_train, bone_lengths_test = data_utils.read_3d_data_L(actions, opt.data_dir, opt.camera_frame, rcams, opt.TRAIN_SUBJECTS, opt.TEST_SUBJECTS, opt.predict_14, flag_bone_lengths = config.bone_lengths )  
...
def evaluate_batches_test(opt,model,
  ...
  for i in range(nbatches):
    ...
    dec_out = data_utils.deselect_joints(dec_out, data_mean_3d, dim_to_ignore_3d)
    poses3d = data_utils.deselect_joints(outputs.data.cpu().numpy(), data_mean_3d, dim_to_ignore_3d)
...
def evaluate_batches(opt,
  ...
  # poses3d = data_utils.unNormalizeData( outputs.data.cpu().numpy(), data_mean_3d, data_std_3d, dim_to_ignore_3d )
  dec_out2 = data_utils.deselect_joints(dec_out, data_mean_3d, dim_to_ignore_3d)
  poses3d = data_utils.deselect_joints(outputs.data.cpu().numpy(), data_mean_3d, dim_to_ignore_3d)
